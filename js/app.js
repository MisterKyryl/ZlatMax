(() => {
    ...
    function rangeInit() {
        ...
        if (rangeItems.length) rangeItems.forEach((rangeItem => {
            ...
            initialize(item, {
                start: [ Number(fromValue.value.replace(" грн.", "")), Number(toValue.value.replace(" грн.", "")) ],
                connect: true,
                tooltips: [ {
                    to: function(value) {
            ...
            item.noUiSlider.on("update", (function(values, handle) {
                fromValue.value = Math.round(values[0]) + " грн.";
                toValue.value = Math.round(values[1]) + " грн.";
            }));
        }));
        const rangeLengthItems = document.querySelectorAll("[data-length-range]");
        if (rangeLengthItems.length) rangeLengthItems.forEach((rangeLengthItem => {
            const fromValue = rangeLengthItem.querySelector("[data-length-range-from]");
            const toValue = rangeLengthItem.querySelector("[data-length-range-to]");
            const item = rangeLengthItem.querySelector("[data-length-range-item]");
            initialize(item, {
                start: [ Number(fromValue.value), Number(toValue.value) ],
                connect: true,
                range: {
                    min: [ Number(fromValue.dataset.lengthRangeFrom) ],
                    max: [ Number(toValue.dataset.lengthRangeTo) ]
                },
                format: {
                    to: function(value) {
                        return Math.round(value);
            ...
            item.noUiSlider.on("update", (function(values, handle) {
                if (handle === 0) fromValue.value = Math.round(values[handle]); else if (handle === 1) toValue.value = Math.round(values[handle]);
            }));
            function updateSliderFromInput() {
                item.noUiSlider.set([ Number(fromValue.value), Number(toValue.value) ]);
            }
            fromValue.addEventListener("change", updateSliderFromInput);
            fromValue.addEventListener("blur", updateSliderFromInput);
            toValue.addEventListener("change", updateSliderFromInput);
            toValue.addEventListener("blur", updateSliderFromInput);
        }));
    }
    rangeInit();
    ...
    function initSliders() {
        ...
        if (document.querySelector(".products-flashligh__slider")) new swiper_core_Swiper(".products-flashligh__slider", {
            modules: [ Navigation, Pagination, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
